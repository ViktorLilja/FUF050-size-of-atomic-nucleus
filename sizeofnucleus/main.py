...
exp_cs    = np.array(CSV_data[header[1]]) * (1e-3 * nc.barn)
exp_error = np.array(CSV_data[header[2]]) * (1e-3 * nc.barn)
...
E = 250e6 * nc.eV   # Energy of electrons
A = 40              # Number of nucleons in nucleus 
Z = 20              # Number of protons in nucleus
...
# ---------------------------------------------------#
# --- CALCULATE PROPERTIES OF CHARGE DISTIBUTION --- #
# -------------------------------------------------- #
...
# Print RMS
rms_acc         = 3.4776 * nc.fm           # Accepted rms
...
print("Expected RMS radius %f fm" % (rms_acc / nc.fm))
print("Calculated RMS radius %f fm, relative error %.2f%%" 
       % (rms_radius(rho_Ch_star)/nc.fm, 100*rms_rel_error))
...
plt.plot(theta / deg, 
         cs_star / (1e-3 * nc.barn),
         "k--")
plt.errorbar(exp_theta / deg, 
             exp_cs / (1e-3 * nc.barn), 
             yerr = exp_error / (1e-3 * nc.barn),
             fmt = "r.")
plt.yscale('log')
...
r = np.linspace(0, 8) * nc.fm
plt.plot(r*1e15, 
         rho_Ch_star(r) / (nc.e * nc.fm**-3))
...
with open('sizeofnucleus/output/result.csv', 'w') as f:
    ...
    for i in range(0, rows):
        ...
        f.write(",")
        if (len(exp_cs)         > i) : f.write("%e" % (exp_cs[i] / (1e-3 * nc.barn)))
        f.write(",")
        if (len(exp_error)      > i) : f.write("%e" % (exp_error[i] / (1e-3 * nc.barn)))
        f.write(",")
        if (len(theta)          > i) : f.write("%e" % (theta[i] / deg))
        f.write(",")
        if (len(cs_star)        > i) : f.write("%e" % (cs_star[i] / (1e-3 * nc.barn)))
        f.write(",")
        if (len(r)              > i) : f.write("%e" % (r[i] / nc.fm))
        f.write(",")
        if (len(rho_Ch_star(r)) > i) : f.write("%e" % (rho_Ch_star(r)[i] / (nc.e * nc.fm**-3)))
        f.write("\n")
